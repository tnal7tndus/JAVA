...
<c:if test="${!empty requestScope.apple}">
	<!-- ** idbList(id별 boardList) 
		=> 선택된 id를 function에 전달(매개변수를 활용)
		   idbList('apple')
		=> a Tag 이용 이벤트 적용시 책갈피 기능 활용 : 스크롤 이동 가능
			-> href : 적용하지 않음(이동하지 않음)
			-> href="#id" : id 위치로 이동
			-> "#" : 최상단으로 이동 
			-> href="javascript:;" : 자바스크립트를 입력하지 않는다는 문구. 이동하지 않음
	-->
	<c:forEach var="v" items="${requestScope.apple}">
		<tr><td onclick="idbList('${v.id}')">${v.id}</td>
		<td>${v.name}</td><td align="center">${v.age}</td>
		
		<!-- ** Jo정보 Div에 출력하기 -->
		<td align="center">
			<span class="textlink" onmouseover="showJoDetail(event, ${v.jno})"
								   onmouseout="hideJoDetail()">${v.jno}</span>
		</td>
	...
	<!-- ** Delete 기능 추가
		...
		-->
		<td><span id="${v.id}" class="textlink" onclick="axiDelete(event,'${v.id}')">Delete</span></td>
		</tr>
	</c:forEach>
