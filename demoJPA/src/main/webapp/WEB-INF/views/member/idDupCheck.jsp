...
//2) 부모창의 ID중복확인 버튼은 disable & submit은 enable
//3) 부모창의 id는 수정불가 (readonly), password에 focus
//4) 현재(this)창은 close
function idOK(){
	//1)
	//opner.document.getElementById('id').value=document.getElementById('id').value;
	opener.document.getElementById('id').value="${param.id}";
	// => EL 활용: jsp 문서에서는 script 구문의 문자열 내부에 있는 EL은 처리해줌
	// window.jQuery.attr() -> window는 생략 가능/ jQuery = $ => $.attr() 로 사용함
	//2)
	...
	//4)
	close(); //window.close() /  self.close() 모두 가능
}//idOK
