...
// 	 Page<Entity> Type으로 return하기 때문에 서비스계층에서 이를 처리할 수 있도록 하는 DTO클래스가 필요함.
//=> 주요기능
...
//	- Collection과 관련된 편리한 메서드를 제공 -> Collections.sort(List<T> list)
...
public class PageResultDTO<DTO, EN> {
	...
	private int page; // 현재 출력 Page
	private int size; // 1 Page당 출력 row 갯수
	
	private int start, end; // 이전, 다음 목록
	private boolean prev, next;
	
	private List<Integer> pageList; // PageNo의 목록
	
	//** 생성자
    //=> Page<EN> type을 이용해 최종 List<DTO> 생성 
    ...
    private void makePageList(Pageable pageable){
        ...
        //=> rangeClosed() : start ~ end까지 즉, 종료값 포함 return 
