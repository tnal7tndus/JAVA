...
public class MemberDSLRepositoryImpl implements MemberDSLRepository {
	...
	public List<MemberDTO> findMemberJoinDSL() {
		return jPAQueryFactory.select(
				Projections.bean(MemberDTO.class,
						member.id, member.name, member.jno, jo.jname, jo.project))	//bean 종료
				.from(member)
				.leftJoin(jo)
	...
	//    그러므로 DTO에 setter/getter 없어도 가능하며
	...
	public List<MemberDTO> findMemberJoinDSL2() {
		return jPAQueryFactory.select(
				Projections.fields(MemberDTO.class,
						member.id, member.name, member.jno, jo.jname, jo.project))	//bean, select 종료
				.from(member)
				.leftJoin(jo)
				.on(member.jno.eq(jo.jno))
				.fetchJoin()
