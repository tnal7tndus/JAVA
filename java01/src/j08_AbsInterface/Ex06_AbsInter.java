...
//   이런 경우 interface 를 구현하는 추상클래스를 작성하고
//   게시판은이 추상클래스를 상속.구현한다면 의무적 구현에서 벗어나 필요한 메서드만 구현 할 수 있음.
...
// => 선택적으로 오버라이딩 가능
// => 의무구현 메서드(i,l)는 추상메서드로 두고, 선택적 구현메서드들은 미리 구현해놓음
...
public class Ex06_AbsInter {
	...
	public static void main(String[] args) {
		...
		// => BoardA를 상속받은 NoticeBoard()와는 교체 가능
		// => BoardA를 상속받지 않은 MyBoard로는 생성할수없음 
		ba.listPrint();
		ba.delete(); 
		// => QnABoard에는 정의하지 않았지만, 조상인 BoardA에 있으므로 가능함.
		// => 그러나 BoardA에는 없고, QnABoard에만 정의한 메서드가 있다면 사용불가능.
