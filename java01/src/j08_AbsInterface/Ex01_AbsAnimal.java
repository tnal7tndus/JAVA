...
//** 추상 클래스 
//=> 추상메서드가 하나라도 정의되면 그 클래스는 반드시 추상클래스로 정의해야 함 
...
class Cat extends Animal {
	...
	@Override
	void special() { System.out.println("** special: 고양이 eyeColor는 멋집니다 ~~"); }
	
	void eyeColor() { System.out.println("~~ 고양이 eyeColor는 멋집니다 ~~"); }	
} //Cat
...
public class Ex01_AbsAnimal {
	...
	public static void main(String[] args) {
		...
		//    접근 가능한 맴버의 범위는 Type에 정의된 범위로 제한됨
		//    즉, Cat에만 정의된 메서드는 Animal Type에서는 접근할 수 없음
		// => 그러나 실행은 Cat의 메서드가 실행됨.
		//   ( Animal에는 header 부분만 있기 때문에 실행이 불가능함 )
		
		// => 장점
		//	-> Animal Type으로 선언하게되면 우측을 Cat, Dog만 변경하면
		//     클래스교체를 쉽게 할 수 있음
		...
		// => 추상뿐만 아니라 상속관계에서는 모두 적용가능함 
		// => 추상을 사용하면 의무구현의 강제성을 줄수있음 (메서드명을 일원화 할 수 있음)  
		
		// => Animal Type은 생성자의 종류에 따라 다른 객체가 될 수있다.
		// => Animal Type으로 정의했다는 의미는 우측에 누가 오던
		//    Animal에 정의된 맴버만 사용했다는 의미이므로 클래스 교체 가능 
		...
		// ** 연산자 instanceof
		// => 인스턴스 확인 연산자 
		if ( new Cat() instanceof Animal ) System.out.println("** c2는 Animal 인스턴스 입니다 **");
		else System.out.println("** c2 는 Animal 인스턴스가 아닙니다 **");
		
		// => Animal처럼 추상클래스 또는 클래스 Type을 확인하는 경우에는
		//    컴피일 오류 발생 instanceof 연산자의 필요성이 없지만,
		//    interface의 경우에는 런타임시에 오류 발생하므로 instanceof 연산자를 이용한 확인이 필요함 
