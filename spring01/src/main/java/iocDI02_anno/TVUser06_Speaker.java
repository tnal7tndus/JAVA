...
//=> 적용순위
//1) 주로 멤버변수 위에 선언하며,
//   스프링 컨테이너는 해당변수의 타입을 체크하고, 그 타입의 객체가 메모리에 존재하는지 확인하여
//	 해당 변수에 대입 해준다.
...
//** required 속성
//=> true : 해당 타입의 등록된 빈이 없을때 주입시 익셉션이 발생 (default값)
//=> false: 해당 타입의 등록된 빈이 없을때 익셉션이 발생하지 않음 
//  		 ( 있으면 주입 , 없으면 null return )    
//
...
//	=> null return : 실행문에서 접근시 NullPointerException 발생

// ** Test1) : Speaker를 생성하지 않고
// ** Test2) : Speaker를 @으로 생성
// ** Test3) : Speaker를 xml로 생성(xml, @ 병행사용 Test)
...
//   못찾으면 Exception을 발생하지않고 null return 
