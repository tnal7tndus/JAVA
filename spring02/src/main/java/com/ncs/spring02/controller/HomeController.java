...
//** Logger : 현재 위치 상태를 보여줘서 에러 위치를 잡을 수 있게 해줄 수 있는 코드
//=> Log4J의 핵심 구성 요소로 6개의 로그 레벨을 가지고 있으며,
//   출력하고자하는 로그 메시지를 Appender (log4j.xml 참고)에게 전달한다.

//=> 활용 하려면 pom.xml에 dependency (log4j, slf4j) 추가 (되어있음),
//=> Controller에는 아래의 코드를 넣어주고,
//=> 확인이 필요한 위치에서 원하는 메세지 출력, Sysout은 (I/O 발생으로) 성능 저하 유발
...
public class HomeController {
	...
	public String home(Locale locale, Model model) {
		...
		** Logger Message Test
		// 1) {} 활용
		String name="홍길동"; int age=100;
		logger.info("test1) 안녕하세요 ~~");
		logger.info("test2) 안녕하세요 {}님, age={} ~~", name, age);
	...
	@GetMapping("/bcrypt")
	   ...
	   // => web.xml
	   //   : Exception Type은 전달되어 해당하는 jsp는 실행되지만, 
	   //   : page 디렉티브의 isErrorPage="true"와 무관하게 ~.jsp에 ${exception.message} 전달안됨. 
	   
	   // * Spring exceptionResolver 적용  
	   // => servlet~~~.xml에 설정
	   // => page 디렉티브의 isErrorPage="true"와 무관하게 ~.jsp에 ${exception.message} 전달됨. 
	   //    그러나 Spring exceptionResolver가 오류 메시지를 받아 처리하므로 
	   //    console에는 Exception Message가 출력(전달) 되지않는다.
	   //    ( DEBUG level Message는 출력됨. )	
		...
		public String etest(HttpServletRequest request) {
		      ...
		      // => getParameter() : parameter가 없으면 null return
		      ...
		      // 5) SQL Test : Transaction 또는 join으로 			
